...
export async function scrapeNzxt(url: string): Promise<scraperRawResults> {
  ...
  const baseProductInfo =
    pageInfo.props.pageProps.data.decoratedDefaultUpgradeProducts[0];
  ...
  const performance: scraperRawResults["performance"] = Object.keys(
    ...
    const getValue = (value: string) => value ? Number(value) : 0;
    if (game !== "timeSpyOverallScore" && ! (game in gameAcronymMap)) throw Error(`Game ${game} not found in map`);
