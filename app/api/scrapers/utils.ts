...
async function serializeProduct<T extends keyof PrismaModelMap>(
    ...
    if (typeof separator !== 'string') serialized.product_name = title.substring(0, await separator(page))
    ...
    if (serialized.product_name?.includes(serialized.brand as string)) serialized.product_name = serialized.product_name?.split(serialized.brand as string)[1].trim()
    ...
    switch (productType) {
        ...
        case 'caseFan':
            return await saveCaseFan(serialized as unknown as PrismaModelMap['caseFan'])
        default:
            break;
    }
}